 ...
 *
 * 六、使用 suspend 关键字将一个函数声明成挂起函数
 * 1.suspend 关键字能将一个函数声明成挂起函数
 * 2.挂起函数必须在协程或者另一个挂起函数里被调用
 *
 * 七、使用 coroutineScope 函数创建一个协程作用域
 * 1.coroutineScope 函数会继承外部的协程作用域并创建一个子作用域
 * 2.coroutineScope 函数也是一个挂起函数，因此我们可以在任何其他挂起函数中调用
 *
 * 八、使用 async 函数创建一个子协程并获取执行结果
 * launch 函数可以创建一个子协程，但是 launch 函数只能用于执行一段逻辑，却不能获取执行的结果，因为它的返回值永远是一
 * 个 Job 对象，那么如果我们想创建一个子协程并获取它的执行结果，我们可以使用 async 函数。
 * 1.async 函数必须在协程作用域下才能调用
 ...
 *
 * 九、使用 withContext 函数构建一个简化版的 async 函数
 ...
 *
 * 十、使用 suspendCoroutine 函数简化回调的写法
